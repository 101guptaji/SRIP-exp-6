...
public abstract class Statistics
{
  ...
  public static double calculateMean(Number[] values, boolean includeNullAndNaN)
  {
    ...
    int counter = 0;
    for (int i = 0; i < values.length; i++) { double current;
      double current;
      if (values[i] != null) {
    ...
    if (values != null) {
      ...
      if (count > 0) {
        if (count % 2 == 1) {
          ...
          result = (value1.doubleValue() + value2.doubleValue()) / 2.0D;
        }
      }
    }
  ...
  public static double getSlope(Number[] xData, Number[] yData)
  {
    ...
    double sx = 0.0D;double sxx = 0.0D;double sxy = 0.0D;double sy = 0.0D;
    
    for (int counter = 0; counter < xData.length; counter++) {
      sx += xData[counter].doubleValue();
      sxx += Math.pow(xData[counter].doubleValue(), 2.0D);
  ...
  public static double getCorrelation(Number[] data1, Number[] data2)
  {
    ...
    return (n * sumXY - sumX * sumY) / Math.pow((n * sumX2 - sumX * sumX) * (n * sumY2 - sumY * sumY), 0.5D);
  }
