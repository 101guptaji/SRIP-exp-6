  ...
  implements XYItemLabelGenerator, Cloneable, PublicCloneable, Serializable
{
  ...
  protected Object[] createItemArray(XYDataset dataset, int series, int item)
  {
    ...
    if (xdf != null) {
      result[1] = xdf.format(new Date(x));
      result[2] = xdf.format(new Date(xs));
      result[3] = xdf.format(new Date(xe));
    }
    else {
    ...
    if ((Double.isNaN(y)) && (dataset.getY(series, item) == null)) {
      ...
      result[4] = ydf.format(new Date(y));
    }
    else {
    ...
    if ((Double.isNaN(ys)) && (intervalDataset.getStartY(series, item) == null))
    {
      ...
      result[5] = ydf.format(new Date(ys));
    }
    else {
    ...
    if ((Double.isNaN(ye)) && (intervalDataset.getEndY(series, item) == null))
    {
      ...
      result[6] = ydf.format(new Date(ye));
    }
    else {
