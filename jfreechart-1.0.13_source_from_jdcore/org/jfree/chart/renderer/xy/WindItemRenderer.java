  ...
  implements XYItemRenderer, Cloneable, PublicCloneable, Serializable
{
  ...
  public void drawItem(Graphics2D g2, XYItemRendererState state, Rectangle2D plotArea, PlotRenderingInfo info, XYPlot plot, ValueAxis domainAxis, ValueAxis rangeAxis, XYDataset dataset, int series, int item, CrosshairState crosshairState, int pass)
  {
    ...
    double rax2 = x.doubleValue() + windForce * Math.cos(wdirt) * 8000000.0D;
    ...
    double ralx2 = wforce.doubleValue() * Math.cos(aldir) * 8000000.0D * 0.8D + x.doubleValue();
    
    double raly2 = wforce.doubleValue() * Math.sin(aldir) * 0.8D;
    ...
    double rarx2 = wforce.doubleValue() * Math.cos(ardir) * 8000000.0D * 0.8D + x.doubleValue();
    
    double rary2 = wforce.doubleValue() * Math.sin(ardir) * 0.8D;
