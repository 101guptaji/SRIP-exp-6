  ...
  implements XYItemRenderer, PublicCloneable
{
  ...
  protected void drawItemPass0(Graphics2D x_graphics, Rectangle2D x_dataArea, PlotRenderingInfo x_info, XYPlot x_plot, ValueAxis x_domainAxis, ValueAxis x_rangeAxis, XYDataset x_dataset, int x_series, int x_item, CrosshairState x_crosshairState)
  {
    ...
    if (b_impliedZeroSubtrahend) {
      ...
      l_subtrahendCurY = new Double(0.0D);
      l_subtrahendNextY = new Double(0.0D);
    ...
    double l_x1 = 0.0D;double l_y1 = 0.0D;
    double l_x2 = 0.0D;double l_y2 = 0.0D;
    double l_x3 = 0.0D;double l_y3 = 0.0D;
    double l_x4 = 0.0D;double l_y4 = 0.0D;
    ...
    while ((!b_minuendDone) && (!b_subtrahendDone)) {
      ...
      else
      {
        ...
        else
        {
          ...
          if ((0.0D < l_slopeA) && (l_slopeA <= 1.0D) && (0.0D < l_slopeB) && (l_slopeB <= 1.0D))
          {
      ...
      if (b_intersect)
      {
        ...
        boolean b_positive = (l_subtrahendMaxY <= l_minuendMaxY) && (l_subtrahendMinY <= l_minuendMinY);
  ...
  protected void drawItemPass1(Graphics2D x_graphics, Rectangle2D x_dataArea, PlotRenderingInfo x_info, XYPlot x_plot, ValueAxis x_domainAxis, ValueAxis x_rangeAxis, XYDataset x_dataset, int x_series, int x_item, CrosshairState x_crosshairState)
  {
    ...
    if (null != l_entities) {
      if (null == l_entityArea) {
        l_entityArea = new Rectangle2D.Double(l_x1 - 2.0D, l_y1 - 2.0D, 4.0D, 4.0D);
      }
  ...
  public boolean equals(Object obj)
  {
    ...
    if (!PaintUtilities.equal(positivePaint, positivePaint)) {
      return false;
    }
    if (!PaintUtilities.equal(negativePaint, negativePaint)) {
      return false;
    }
    ...
    if (roundXCoordinates != roundXCoordinates) {
      return false;
    }
    return true;
  }
    ...
    throws IOException
  {
    ...
    SerialUtilities.writePaint(positivePaint, stream);
    SerialUtilities.writePaint(negativePaint, stream);
    ...
    throws IOException, ClassNotFoundException
  {
    ...
    positivePaint = SerialUtilities.readPaint(stream);
    negativePaint = SerialUtilities.readPaint(stream);
