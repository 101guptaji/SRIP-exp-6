  ...
  implements Serializable
{
  ...
  {
    seriesBarWidthList = new ObjectList();
  }
  ...
  public double getSeriesBarWidth(int series)
  {
    ...
    Number n = (Number)seriesBarWidthList.get(series);
    if (n != null) {
  ...
  {
    seriesBarWidthList.set(series, new Double(width));
  }
  ...
  protected void drawHorizontalItem(Graphics2D g2, CategoryItemRendererState state, Rectangle2D dataArea, CategoryPlot plot, CategoryAxis domainAxis, ValueAxis rangeAxis, CategoryDataset dataset, int row, int column)
  {
    ...
    rectY += (1.0D - widthFactor) * state.getBarWidth() / 2.0D;
    if (seriesCount > 1) {
      shift = rectHeight * 0.2D / (seriesCount - 1);
    }
    
    Rectangle2D bar = new Rectangle2D.Double(rectX, rectY + (seriesCount - 1 - row) * shift, rectWidth, rectHeight - (seriesCount - 1 - row) * shift * 2.0D);
  ...
  protected void drawVerticalItem(Graphics2D g2, CategoryItemRendererState state, Rectangle2D dataArea, CategoryPlot plot, CategoryAxis domainAxis, ValueAxis rangeAxis, CategoryDataset dataset, int row, int column)
  {
    ...
    rectX += (1.0D - widthFactor) * state.getBarWidth() / 2.0D;
    if (seriesCount > 1)
    {
      shift = rectWidth * 0.2D / (seriesCount - 1);
    }
    
    Rectangle2D bar = new Rectangle2D.Double(rectX + (seriesCount - 1 - row) * shift, rectY, rectWidth - (seriesCount - 1 - row) * shift * 2.0D, rectHeight);
