  ...
  implements ValueAxisPlot, Pannable, Zoomable, RendererChangeListener, Cloneable, PublicCloneable, Serializable
{
  ...
  private transient Point2D quadrantOrigin = new Point2D.Double(0.0D, 0.0D);
  

  private transient Paint[] quadrantPaint = { null, null, null, null };
  ...
  public void draw(Graphics2D g2, Rectangle2D area, Point2D anchor, PlotState parentState, PlotRenderingInfo info)
  {
    ...
    if ((!domainCrosshairLockedOnData) && (anchor != null)) { double xx;
      double xx;
      if (orient == PlotOrientation.VERTICAL) {
  ...
  protected void drawQuadrants(Graphics2D g2, Rectangle2D area)
  {
    ...
    Rectangle2D[] r = { null, null, null, null };
    if ((quadrantPaint[0] != null) && 
      (x > xmin) && (y < ymax)) {
      if (orientation == PlotOrientation.HORIZONTAL) {
  ...
  public boolean equals(Object obj)
  {
    ...
    if (domainMinorGridlinesVisible != domainMinorGridlinesVisible)
    {
      return false;
    }
    ...
    if (domainZeroBaselineVisible != domainZeroBaselineVisible) {
      return false;
    }
    ...
    if (domainCrosshairValue != domainCrosshairValue) {
      return false;
    }
    if (domainCrosshairLockedOnData != domainCrosshairLockedOnData)
    {
      return false;
    }
    ...
    if (!ObjectUtilities.equal(rangeAxes, rangeAxes)) {
      return false;
    }
    ...
    if (!ObjectUtilities.equal(domainMinorGridlineStroke, domainMinorGridlineStroke))
    {
      return false;
    }
    if (!PaintUtilities.equal(domainMinorGridlinePaint, domainMinorGridlinePaint))
    {
      return false;
    }
    ...
    if (!PaintUtilities.equal(domainZeroBaselinePaint, domainZeroBaselinePaint))
    {
      return false;
    }
    if (!ObjectUtilities.equal(domainZeroBaselineStroke, domainZeroBaselineStroke))
    {
      return false;
    }
    ...
    if (!PaintUtilities.equal(domainTickBandPaint, domainTickBandPaint))
    {
      return false;
    }
    if (!PaintUtilities.equal(rangeTickBandPaint, rangeTickBandPaint))
    {
      return false;
    }
    if (!quadrantOrigin.equals(quadrantOrigin)) {
      return false;
    }
    for (int i = 0; i < 4; i++) {
      if (!PaintUtilities.equal(quadrantPaint[i], quadrantPaint[i]))
      {
        return false;
      }
    }
    ...
    throws CloneNotSupportedException
  {
    ...
    domainAxes = ((ObjectList)ObjectUtilities.clone(domainAxes));
    for (int i = 0; i < domainAxes.size(); i++) {
      ValueAxis axis = (ValueAxis)domainAxes.get(i);
      if (axis != null) {
        ValueAxis clonedAxis = (ValueAxis)axis.clone();
        domainAxes.set(i, clonedAxis);
    ...
    rangeAxes = ((ObjectList)ObjectUtilities.clone(rangeAxes));
    for (int i = 0; i < rangeAxes.size(); i++) {
      ValueAxis axis = (ValueAxis)rangeAxes.get(i);
      if (axis != null) {
        ValueAxis clonedAxis = (ValueAxis)axis.clone();
        rangeAxes.set(i, clonedAxis);
        clonedAxis.setPlot(clone);
        clonedAxis.addChangeListener(clone);
      }
    }
    rangeAxisLocations = ((ObjectList)ObjectUtilities.clone(rangeAxisLocations));
    


    datasets = ((ObjectList)ObjectUtilities.clone(datasets));
    for (int i = 0; i < datasets.size(); i++) {
    ...
    renderers = ((ObjectList)ObjectUtilities.clone(renderers));
    for (int i = 0; i < renderers.size(); i++) {
    ...
    foregroundDomainMarkers = ((Map)ObjectUtilities.clone(foregroundDomainMarkers));
    
    backgroundDomainMarkers = ((Map)ObjectUtilities.clone(backgroundDomainMarkers));
    
    foregroundRangeMarkers = ((Map)ObjectUtilities.clone(foregroundRangeMarkers));
    
    backgroundRangeMarkers = ((Map)ObjectUtilities.clone(backgroundRangeMarkers));
    ...
    quadrantOrigin = ((Point2D)ObjectUtilities.clone(quadrantOrigin));
    
    quadrantPaint = ((Paint[])quadrantPaint.clone());
    return clone;
  }
